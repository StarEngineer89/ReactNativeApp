...
import { Linking, Platform, ScrollView, Text, TouchableOpacity } from 'react-native';
...
const HomeDrawerMenu = ({ authType, ...props }) => {
  ...
  return (
    <SafeAreaView edges={['left', 'right', 'top', 'bottom']} style={[{ flex: 1, backgroundColor: palette.primary }]}>
      <VStack style={[{ flex: 1, backgroundColor: 'transparent' }]} spacing={5}>
        ...
        <ScrollView
          style={{ flex: 1, backgroundColor: 'transparent', width: Platform.isPad ? 480 : 230 }}
          // bounces={false}
            ...
            <VStack spacing={2} style={{ backgroundColor: 'transparent', paddingHorizontal: 5 }}>
              ...
              <ExpandableList title='About the app'>
                <Text style={[StyleGuide.typography.menuSubItem]}>
                  We aim at making language learning a fun experience while preserving the charming characteristics of
                  local dialects. We are here to help you reconnect with your roots and discover new cultures together.
                </Text>
                {authType === 1 && (
                  <MenuItem title={'Contact us'} onPress={() => navigationRef.current?.navigate(DRAWER.CONTACT_US)} />
                )}
              </ExpandableList>
