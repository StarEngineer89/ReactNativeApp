...
import {
  ...
  SpeakerVolume,
  PageItem,
} from 'components/main/RecordingComponents';
...
const StudentClassSetScreen = ({ route, navigation }) => {
  ...
  const onEndRecording = useCallback(
    (uri) => {
      ...
      recordVoice(state._id, item._id, cid, catId, predefined, uri, item.studentVoiceURL);
    },
    [state.currentCategory]
  );
  ...
  return state.currentCategory == null ? (
    ...
    >
      ...
      <View style={[{ position: 'absolute', bottom: tickerBottom }]}>
        <TickerView translateX={translateX} tWidth={Platform.isPad ? 280 : 200} tHeight={50}>
                ...
                {[...Array(5).keys()].map((index) => {
                  return (
                    <Image
                      ...
                      style={{
                        width: Platform.isPad ? 28 : 20,
                        height: Platform.isPad ? 26 : 19,
                      }}
      ...
      <Arrow
        ...
        stopIndex={state.currentCategory.children.length - 1}
        onArrowPressed={onNextPressed}
      />
      ...
      <SpeakerVolume
        data={state.currentCategory.children}
        activeIndex={activeIndex}
        attr={'studentVoiceURL'}
        onPress={onStudentSpeakerPressed}
      />

      <MicroPhoneRecorder
        onStart={() => lottieRef.current.play()}
        onEnd={() => lottieRef.current.reset()}
        onFinished={onEndRecording}
      />
