...
interface Props
  extends CompositeScreenProps<
    NativeStackScreenProps<ISetStackNavigatorParamsList, SETS.EDIT>,
...
const CustomSetEditScreen = ({ route }: Props) => {
  ...
  return (
    <ContainerView>
      ...
      {state.savingSet.error && <ErrorModal clearAction={() => clearError('SET')} errorText={state.savingSet.error} />}
      <ScrollView>
        <VStack spacing={15}>
          <ImageUploader source={customSet.image} onSelectImage={uri => setCustomSet({ ...customSet, image: uri })} />
          <LabelTextInput label="Change Set Name" value={customSet.name} onChangeText={value => setCustomSet({ ...customSet, name: value })} />
          ...
          <View
            ...
            <Button
              ...
              title="Save"
              onPress={() => {
                editSet(id, customSet.name, customSet.image, customSet.image != _set.image);
              }}
            />
          </View>
        </VStack>
