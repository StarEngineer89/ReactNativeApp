...
const SetItemEditScreen = ({ route, navigation }) => {
  ...
  return (
    ...
    >
      ...
      <Arrow
        ...
        stopIndex={state.currentCategory.length - 1}
      ...
      <View style={{ position: 'absolute', top: 120 }} pointerEvents='none'>
        <TickerView translateX={translateX} tWidth={width} tHeight={50}>
          {state.currentCategory.map(({ name, slug }, index) => {
            return (
              <View
      ...
      <View style={{ position: 'absolute', bottom: StyleGuide.main.isSmallDevice ? 20 : 50 }}>
        <Button
          ...
          variant='gradient'
          title={'Done'}
          onPress={() => navigation.navigate(SETS.DETAILS, { itemIndex: activeIndex.value })}
        />
      </View>
    </View>
  );
