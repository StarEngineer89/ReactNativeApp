...
interface Props
  extends CompositeScreenProps<
    NativeStackScreenProps<IStudentStackNavigatorParamsList, STUDENTS.EDIT>,
...
const StudentEditScreen = ({ navigation, route }: Props) => {
  ...
  useEffect(() => {
    const unsubscribeFocus = navigation.addListener('focus', () => {
      if (route.params?.sets) setStudent({ ...student, categories: route.params.sets });
    });
  ...
  return (
    <ContainerView>
      ...
      {state.savingStudent.error && <ErrorModal clearAction={() => clearError('STUDENT')} errorText={state.savingStudent.error} />}
      <ScrollView>
        <VStack spacing={15}>
          <ImageUploader source={student.image} onSelectImage={uri => setStudent({ ...student, image: uri })} />
          <LabelTextInput label="Change Student Name" value={student.name} onChangeText={value => setStudent({ ...student, name: value })} />
        </VStack>
        <VStack style={{ flex: 1 }} spacing={2} alignment="leading">
          <HStack style={{ paddingLeft: isTablet || isLandscape ? 45 : 30 }}>
            <Pressable
              ...
              <HStack alignment="center" style={{ backgroundColor: 'transparent' }} spacing={5}>
                <AddButton size="xs" disabled />
                <Text style={StyleGuide.typography.inputLabel}>Select Sets</Text>
              </HStack>
            </Pressable>
            <Spacer />
          </HStack>
          ...
          <HList
            ...
            snapToInterval={4 * (StyleGuide.sizes.thumb.sm.width + 2 * StyleGuide.main.spacing)}
            renderItem={({ item }) => (
              <VStack spacing={5}>
                {item.map(({ index }) => {
                  ...
                  return (
                    <Animated.View
                      ...
                      <Image size="sm" uri={_item.image} defaultSource={_item.slug} />
                    </Animated.View>
                  );
                })}
              </VStack>
          ...
          <View
            ...
            <Button
              ...
              title="Save"
              onPress={() => {
                editStudent(id, student.name, student.categories, student.image, student.image != _std.image);
              }}
            />
          </View>
        </VStack>
