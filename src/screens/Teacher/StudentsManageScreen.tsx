...
const StudentsManageScreen = ({ navigation }: Props) => {
  ...
  return (
    <ContainerView>
      ...
      />
      <VStack style={{ flex: 1 }} alignment={isLandscape ? 'leading' : 'center'}>
        {isLandscape && (
          <View
            style={[
              {
              ...
              <Animated.View entering={SlideInLeft.springify().delay(200)} exiting={ZoomOut.delay(200)} layout={Layout.delay(500)}>
                <HStack
                  ...
                  <VStack spacing={1} style={isLandscape && { marginTop: 10 }}>
                    <Button title="Edit Student" variant="outline" size="md" onPress={() => navigation.navigate(STUDENTS.EDIT, { id: item._id })} />
                    <Button
